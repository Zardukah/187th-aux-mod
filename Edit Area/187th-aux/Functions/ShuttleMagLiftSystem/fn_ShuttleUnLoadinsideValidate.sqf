_vehicles = ((_this select 0) getVariable ["OPTRE_shuttle_AttachedToVehiclesEffect",[]]);
...
if (
	...
	if (((getPosATL (_this select 0)) select 2) < 2) then {
		...
		(_this select 0) setVariable ["OPTRE_shuttle_AttachedToVehiclesEffect", [], true];

		};

} else {
	titleText ["-------------------------------------------<br/><t color='#ff0000' size='1.5'>Cargo Unloaded!</t><br/>-------------------------------------------", "PLAIN DOWN", -1, true, true];
		playSound "FD_Finish_F";

	0 = (_this select 0) spawn OES_fnc_shuttleLoad_UnloadAllSupplyPods;
