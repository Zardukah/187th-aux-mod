...
co18_BDC_LAATCMonitor_AirCargoTypes = ["187th_Nu_A_Class_Shuttle"]; // default: "swop_LAAT_cargo" contingent on classname change
...
co18_BDC_LAATCMonitor_AttachPoint = [0,-2,-1.75]; // default: [0,-2,-1.5]
co18_BDC_LAATCMonitor_SpecificAttachPoints = [ // List of specific object classes or config classes (as a list in array form) that have specific attachment points to the underside of the bird
	...
	[["B_T_Truck_01_Repair_F","B_T_Truck_01_box_F","B_Truck_01_Repair_F","B_Truck_01_box_F"],[0,-2,-3.00]], // HEMTT tall trucks
	[["Republic_ATTE"],[0,-1.65,-7.75]], // Republic AT-TE turret tank monstrosity
	[["SW_FIELDGUN"],[0,-2.5,-5.50]], // Republic large turret - move it back a bit so the main turret isn't sticking thru the cockpit
	[["SW_AV7"],[0,-4,-9]], // AV7
	[["O_JM_TX130r_1","O_JM_TX130m1r_1","O_JM_TX130m2r_1","O_JM_TX130_1","O_JM_TX130m2_1_RD501","O_JM_TX130m2_1"],[0,-1,-4.5]], // Sabre tanks raised up due to short body
	[["Slingload_base_F","Slingload_01_Base_F"],[0,-3,-1.5]], // Cargo containers - move back and up
	[["SCI_B_APC_Wheeled_01_cannon_F"],[0,-2,-1.0]], // 212 Aux Assets Bantha APC
...
co18_BDC_LAATCMonitor_SpecificDetachPoints = [ // List of specific objects that require a specific detachment point (such as vehicles with length turrets) relative to the attached point of that object
	[["Republic_ATTE"],[0,-2.5,-3]],  // Republic AT-TE move it back some to prevent turret from sticking to LAAT cockpit body
	[["SW_AV7"],[0,0,-5.50]], // AV7 turret
	[["Slingload_base_F","Slingload_01_Base_F"],[0,0,-1.75]], // Cargo containers - lower a bit
	[["B_T_Truck_01_Repair_F","B_T_Truck_01_box_F","B_Truck_01_Repair_F","B_Truck_01_box_F"],[0,0,-3.00]], // HEMTT tall trucks
	[["SCT_B_APC_Wheeled_01_cannon_F"],[0,0,-1.5]], // 212th Bantha APC
...
co18_BDC_LAATCMonitor_MinimumLoadAlt = 0; // default: 4
...
co18_BDC_LAATCMonitor_SpecificMinimumDropAlt = [ // Specific minimum terrain altitude drop points for specific vehicle types (short ones that can be loaded on the ground and such)
	[[
		"Slingload_base_F","Slingload_01_Base_F","SCT_B_APC_Wheeled_01_cannon_F",
		"O_JM_TX130r_1","O_JM_TX130m1r_1","O_JM_TX130m2r_1","O_JM_TX130_1","O_JM_TX130m2_1_RD501","O_JM_TX130m2_1",
		"B_T_Truck_01_Repair_F","B_T_Truck_01_box_F","B_Truck_01_Repair_F","B_Truck_01_box_F"
		],0] // Array structure: [["Classname1","Classname2",etc],Minimum drop altitude]
];
	...
	while {true} do {
		...
		if (alive player && (vehicle player != player)) then {
			...
			if (_isDriver && (_VehType in co18_BDC_LAATCMonitor_AirCargoTypes)) then {
				...
				_ParachuteDropEnabled = _Vehicle getVariable ["CargoParachuteEnabled",false];
					// Disable/enable automatic cargo parachute
				if (co18_BDC_LAATCMonitor_ParachuteDropAlt > 0) then {
					if (!_ParachuteDropEnabled) then {
						if (co18_BDC_r_player_EnableParachute < 0) then {
							co18_BDC_r_player_EnableParachute = player addAction ["Enable Parachute for Cargo Drop", {
								(vehicle player) setVariable ["CargoParachuteEnabled",true,true];
						...
						if (co18_BDC_r_player_DisableParachute < 0) then {
							co18_BDC_r_player_DisableParachute = player addAction ["Disable Parachute for Cargo Drop", {
								(vehicle player) setVariable ["CargoParachuteEnabled",false,true];
				...
				if !(isNull _VehViVCargo) then {
					...
					if (co18_BDC_r_player_CheckViVCargo < 0) then {
						...
						// Unload
					_CanUnload = false;
					_HasSpecificMinAlt = false;
					_VehViVCargo = _Vehicle getVariable ["VehViVCargo",objNull];
					{
						_VehArray = _x select 0;
						_DropAlt = _x select 1;
						if (typeOf _VehViVCargo in _VehArray) then {
							if (_vAlt >= _DropAlt) then {
								_CanUnload = true;
								_HasSpecificMinAlt = true;
							};
						};
					} forEach co18_BDC_LAATCMonitor_SpecificMinimumDropAlt;
					if (!_HasSpecificMinAlt && (_vAlt >= co18_BDC_LAATCMonitor_MinimumDropAlt)) then {
						_CanUnload = true;
					};
					if (_CanUnload) then {
						if (co18_BDC_r_player_UnloadViVCargo < 0) then {
							...
							co18_BDC_r_player_UnloadViVCargo = player addAction [_FStr, {
								...
								if ((_vAlt >= co18_BDC_LAATCMonitor_ParachuteDropAlt) && (co18_BDC_LAATCMonitor_ParachuteDropAlt > 15) && _ParachuteDropEnabled) then { // Ping server to create and attach chute
									if (isServer) then {
								...
								sleep 0.5;
								LAATCMonitor_FlagVehicleSimulation = [_VehViVCargo,1];
								publicVariableServer "LAATCMonitor_FlagVehicleSimulation";
					...
					if ((co18_BDC_r_player_nearestViVObject != _VehViVCargo) && (co18_BDC_r_player_nearestViVObject isKindOf "StaticCannon" || co18_BDC_r_player_nearestViVObject isKindOf "StaticMGWeapon" || co18_BDC_r_player_nearestViVObject isKindOf "Car" || co18_BDC_r_player_nearestViVObject isKindOf "Tank" || co18_BDC_r_player_nearestViVObject isKindOf "ReammoBox" || co18_BDC_r_player_nearestViVObject isKindOf "ReammoBox_F" || co18_BDC_r_player_nearestViVObject isKindOf "Slingload_base_F" || (co18_BDC_r_player_nearestViVObject isKindOf "Air" && co18_BDC_LAATCMonitor_IncludeAircraft)) && (_vSpeed <= 15) && (_vAlt >= co18_BDC_LAATCMonitor_MinimumLoadAlt)) then {
						...
						if (co18_BDC_r_player_LoadViVCargo < 0) then {
							co18_BDC_r_player_LoadViVCargo = player addAction [_FStr, {
								...
								LAATCMonitor_FlagVehicleSimulation = [co18_BDC_r_player_nearestViVObject,0];
								publicVariableServer "LAATCMonitor_FlagVehicleSimulation";
								co18_BDC_r_player_nearestViVObject attachTo [_Vehicle, _AttachPoint];
								co18_BDC_r_player_nearestViVObject attachTo [_Vehicle, _AttachPoint]; // test
									// Reverse direction
								if (typeOf co18_BDC_r_player_nearestViVObject in co18_BDC_LAATCMonitor_ReverseObjects) then {
									...
									//co18_BDC_r_player_nearestViVObject setDir 180;
									diag_log format["test reversing object %1 %2",co18_BDC_r_player_nearestViVObject,typeOf co18_BDC_r_player_nearestViVObject];
									LAATCMonitor_ReverseAttachedObject = co18_BDC_r_player_nearestViVObject;
									publicVariableServer "LAATCMonitor_ReverseAttachedObject";
								};
								player removeAction co18_BDC_r_player_LoadViVCargo;
								co18_BDC_r_player_LoadViVCargo = -1;
								co18_BDC_r_player_nearestViVObject = objNull;
